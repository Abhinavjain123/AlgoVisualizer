...
def Astar(draw,grid,start,end):
    ...
    open_set_hash = {start}   # hash for priorityQueue that a element is in it or not
    while not open_set.empty():
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            
        current = open_set.get()[2]  #spot at index 2 in priority queue
        ...
        for neighbor in current.neighbors:
            ...
            if temp_g_score < g_score[neighbor]:
                came_from[neighbor] = current     # came from is a dictionary
